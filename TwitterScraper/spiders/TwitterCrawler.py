...
class TwitterCrawler(CrawlSpider):
    ...
    def __init__(self, query='', lang='', crawl_user=False, top_tweet=False, file_name='tweets'):
        self.query = query
    ...
    def parse_tweet_item(self, items):
        for item in items:
            try:
                ...
                # To get the text data, we first select the text-container and parse it using beautifulsoup:
                text_container = bs(' '.join(item.xpath('.//div[@class="js-tweet-text-container"]/p').extract()),
                                    'html.parser')
                # Emoticons in the text are replaced by their UTF-8:
                for emo in text_container.find_all("img", class_="Emoji Emoji--forText"):
                    emo.insert(0, ' ' + emo["alt"] + ' ')

                if text_container is not None:
                    tweet['text'] = re.sub(' +', ' ',
                                           text_container.get_text().replace("\n", " ").replace("\r", " ").replace("\t",
                                                                                                                   " "))
                else:
                    continue
                ...
                tweet['timezone'] = time.tzname[time.daylight]
